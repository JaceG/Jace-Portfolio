export default function handler(req, res) {
	const projects = [
		{
			...
			description:
				"<strong>⚠️ WORK IN PROGRESS: Please keep checking back for updates as this project evolves!</strong><br><br><strong>Phone Case Store</strong> is a direct-to-consumer ecommerce site for original and licensed phone-case designs. Instead of a generic product grid, every design gets its own landing-page-style product page with a cinematic 3D case preview, a phone picker covering 44 visually approved iPhone and Samsung models, and a shared cart built around repeating three-case sets (three cases for $50, six for $100, nine for $150). Browsing and switching designs never touches the cart, and each item in a set can be a different design on a different phone, so a household can mix and match in a single order.<br><br>The heart of the project is the owner-facing <strong>Catalog Studio</strong>. Adding a design is one artwork upload: the owner drags, zooms and rotates the flat artwork on the print layout, tunes custom fits per phone model, and saves everything as a private draft. Generating previews kicks off local background Blender jobs that render hero, flat and three-quarter product shots for every selected phone, which the owner reviews in private desktop and mobile product-page previews before explicitly publishing to the catalog. Drafts and live revisions coexist, edits keep private history snapshots for render consistency, trashed designs are restorable, and routine catalog copy (taglines, palette colors extracted from the artwork, search titles and descriptions, related designs) is auto-filled deterministically on save.<br><br>From a technical perspective, the storefront, admin and commerce layers run in a single TypeScript app: Next.js with the App Router, Payload CMS and its ecommerce plugin on top of PostgreSQL, styled with Tailwind CSS and animated with Motion and Lenis smooth scrolling. Landing pages pick desktop or mobile layouts through server-side device detection, and the in-browser case viewer is built on three.js with GLB shells. A Python and Blender pipeline builds parametric silicone shells from Apple's published dimensional drawings, repairs UVs, validates openings and lens lips, and batch renders headlessly. Stripe checkout is wired up but gated behind a flag until launch, and the project is covered by Vitest integration tests and Playwright end-to-end tests.",
			image: '/phone-case-store-screenshot.png',
			github: 'https://github.com/JaceG/Phone-Case-Website',
			app: '',
		},
		{
			slug: 'imagedrop',
			...
			description:
				"<strong>ImageDrop</strong> is a tiny macOS menu bar app that makes image conversion a one-drop task. Click the photo icon in the menu bar, drop a file onto the panel (or click to choose one), pick an output format and a compression preset, and the converted copy is saved right next to the original. It reads and writes PNG, JPEG, WebP, HEIC, TIFF, GIF and BMP, and ships with presets tuned for where the image is going: Original, Web page, Social media, Email / messaging and Thumbnail. Everything runs entirely on the Mac, so nothing is ever uploaded anywhere.<br><br>The app is free and open source under the MIT license, and it ships as a complete product rather than just a repo. The website at imagedropapp.com explains the features, walks new users through the Gatekeeper &quot;Open Anyway&quot; step needed for an app that isn't notarized with Apple, and points its download button at the latest GitHub Release so it never goes stale. The site also embeds a Ko-fi tip panel and GitHub Sponsors button for anyone who wants to support the project, with a Windows version planned next.<br><br>From a technical perspective, the entire app is a single ~500 line Swift file with no dependencies and no Xcode project; a shell script compiles it with <strong>swiftc</strong> into a ~200 KB app bundle. It uses AppKit's NSStatusItem and NSPanel for the menu bar drop panel and Apple's ImageIO framework (CGImageSource and CGImageDestination) for decoding and encoding, including HEIC. Because macOS can decode WebP but not encode it, WebP export shells out to Homebrew's cwebp. A companion Swift script generates the app icon, a release script zips a build and publishes it as a GitHub Release, and the marketing site is static HTML served from GitHub Pages on a custom domain.",
			image: '/imagedrop-website-screenshot.png',
			github: 'https://github.com/JaceG/ImageDrop',
			app: 'https://imagedropapp.com/',
		},
		{
			slug: 'remi',
			// Kept for the record but pulled from every public listing, the slug
			// lookup and the case-study route (see the layout's notFound() guard).
			// The stored image is blurred, not the original screenshot.
			hidden: true,
			title: 'REMI',
			snippet:
			...
			description:
				"<strong>REMI</strong> is the operating system for a mobile oil change franchise, built professionally for REMI-OS inc and used daily by MAXI technicians and franchise owners in the field. It started as separate technician and customer apps and was consolidated into one Expo and React Native binary with technician and customer modes, backed by a separate Node, Express and PostgreSQL API. Over a three-month stretch I shipped more than 500 commits to the app, working directly from field reports and keeping a running development log of every change and decision. The source is private, so there are no public links for this one.<br><br>The technician side is where the app earns its keep. A dispatch <strong>Calendar</strong> with route, day, week and month views lets franchise owners drag jobs between technicians on a vendored, customized calendar library. <strong>Start Job</strong> walks a technician through an eleven-step flow: confirm the vehicle by VIN scan, license plate lookup through CARFAX QuickVIN or fleet ID search, pick the customer (including multi-vehicle household and fleet visits), select services and packages with negotiated per-job pricing, choose recommended fluids and filters from MOTOR OEM specs, ShowMeTheParts fitment and live van stock, run a vehicle inspection with tire tread on four- and six-wheel layouts, time the service, review the invoice, collect payment with Square Tap to Pay, and finalize. <strong>Orders</strong> is a searchable ledger with Created, Finalized, Billed, Waiting and Paid states, while <strong>Order Progress</strong> shows a day's open and finalized work side by side. A paginated <strong>Customers</strong> directory handles thousands of records with an A-Z jump bar and a background-warmed, persisted cache. Around that core sit operation-scoped inventory with purchase orders and count sheets, fleet management, end-of-day CARFAX service record files, an AI copilot with voice via the OpenAI Realtime API, a team feed, training modules and daily briefings.<br><br>From a technical perspective, the app is Expo 54 and React Native 0.81 in TypeScript with Expo Router file-based routing, TanStack Query for server state (with a persisted subset that survives restarts), Zustand for client state, React Hook Form with Zod schemas behind every form, Axios with JWT refresh interceptors, background GPS via expo-location and task manager, push notifications, WebSocket subscriptions for realtime updates, and Sentry monitoring. Shared workspace packages cover the API client, auth, permissions, tenant branding and UI. The hardest problem was vehicle scanning: after bake-offs across Dynamsoft, Scandit and Scanbot, VIN capture settled on expo-camera with cloud OCR and plates moved to server-side ALPR because on-device models misread embossed US plate fonts. Releases ship through EAS with over-the-air updates for JavaScript-only changes and a three-app dev, preview and production setup for field testing.",
			image: '/remi-screenshot.png',
			github: '',
			app: '',
		},
		{
		...
		},
		{
			...
			snippet:
				'A business search application, AI-powered search suggestions, and caching to reduce API costs.',
			description:
		...
		},
		{
			...
			app: 'https://sportsbettingcharts.com/home',
		},
		{
	...
	// slug lookup and the `all` feed below — the record stays in the array
	// above so nothing is lost, it's just not reachable from the site.
