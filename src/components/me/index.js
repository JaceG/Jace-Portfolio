...
export default function Me() {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const staticRectBound = staticSvgRef.current.getBoundingClientRect();
  ...
  const handleImageMouseDown = (e) => {
    ...
    const maxHeight = imageContainerRef.current.getBoundingClientRect().bottom;
  ...
  const handleImageMouseMove = (e) => {
    ...
    const imageLeft =
      parseInt(imageContainerRef.current.style.left?.replace("px", "")) || 0;
    const imageTop =
      parseInt(imageContainerRef.current.style.top?.replace("px", "")) || 0;
